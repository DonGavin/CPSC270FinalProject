...
export function useStockfish() {
  ...
  const [bestMovePositions, setBestMovePositions] = useState<{ source: number; target: number } | null>(null);
  const [moveHistory, setMoveHistory] = useState<Array<{ from: number; to: number; piece: string; fen: string }>>([]);
  ...
  const analyzePosition = (boardState: { [key: number]: string | null }, depth = 15, isWhiteTurn = true) => {
  ...
  const getBestMove = (boardState: { [key: number]: string | null }, depth = 15, isWhiteTurn = true) => {
    if (stockfishRef.current && isReady) {
      setIsThinking(true);
  ...
  const recordMove = (
    ...
    if (stockfishRef.current) {
      ...
      setMoveHistory((prev) => [...prev, { from: source, to: target, piece, fen }]);
